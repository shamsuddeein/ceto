...
export function ProductCard({ p }: { p: any }) {
  ...
  return (
    ...
    >
      <div className={`flex aspect-[4/3] items-center justify-center border-b-[3px] border-border ${tintClass(tint)} p-6 relative overflow-hidden`}>
        {p.cover_image ? (
      ...
      <div className="flex flex-col gap-1.5 p-6">
        <h2 className="font-display text-[18px] font-black text-foreground leading-snug line-clamp-2">{p.title}</h2>
        <span className="mt-2 text-lg font-black text-foreground/80">
          {p.currency || '₦'}{Number(p.price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </span>
      </div>
    </Link>
  );
