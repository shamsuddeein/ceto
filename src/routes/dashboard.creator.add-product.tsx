      ...
      <form onSubmit={submit} className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <Card title="Product files">
            <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-[2rem] border-[4px] border-dashed border-border bg-tint-cream p-12 text-center transition-all hover:bg-tint-peach hover:shadow-vibe-sm">
              <div className="rounded-full border-[3px] border-border bg-white p-4 shadow-vibe-sm mb-2">
                <Upload className="h-8 w-8 text-foreground stroke-[2.5]" />
              </div>
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {(["ebook", "course", "template", "service"] as const).map((t) => (
                <button
                  ...
                  key={t}
                  onClick={() => setType(t)}
                  className={`rounded-xl border-[3px] px-4 py-3 text-base font-black capitalize transition-all ${type === t ? "border-border bg-tint-mint text-foreground shadow-vibe-sm translate-x-1" : "border-transparent bg-background text-foreground/70 hover:border-border hover:bg-tint-cream hover:shadow-vibe-sm hover:-translate-y-1"}`}
                >
                  {t}
                </button>
              ))}
          ...
          <Card title="Cover image">
            ...
            <button
              type="button"
              className="mt-4 w-full rounded-xl border-[3px] border-border bg-white py-3 text-base font-black text-foreground shadow-vibe-sm transition-transform hover:-translate-y-1 hover:bg-tint-peach"
            >
              Upload cover
            </button>
          </Card>
          <button
            type="submit"
            disabled={loading}
            className="inline-flex w-full items-center justify-center gap-2 rounded-full border-[3px] border-border bg-primary py-4 text-lg font-black text-white shadow-vibe hover:-translate-y-1 hover:shadow-vibe-hover disabled:opacity-70 transition-transform"
          >
