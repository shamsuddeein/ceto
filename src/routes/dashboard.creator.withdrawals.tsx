  ...
  return (
    <DashboardLayout title="Withdraw Funds">
      <div className="grid gap-8 lg:grid-cols-[1fr_400px]">
        <form
          onSubmit={submit}
          className="rounded-[2.5rem] border-[4px] border-border bg-white p-6 sm:p-8 shadow-vibe flex flex-col justify-between"
        >
          <div>
            ...
            <div className="mt-8 space-y-6">
              <label className="block">
                <span className="mb-2 block text-base font-black text-foreground">
                  Amount (NGN)
                </span>
                <div className="flex items-center rounded-2xl border-[3px] border-border bg-background shadow-vibe-sm transition-all focus-within:translate-x-[2px] focus-within:translate-y-[2px] focus-within:shadow-none overflow-hidden">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    className="w-full bg-transparent px-3 py-4 font-bold text-foreground outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => setAmount(String(balance))}
                    className="mr-3 rounded-lg border-[3px] border-border bg-tint-mint px-3 py-1.5 text-xs font-black shadow-vibe-sm hover:-translate-y-1 transition-transform"
                  >
                    MAX
                  </button>
                </div>
                <span className="mt-2 block text-sm font-bold text-foreground/60">
                  Available: ₦{balance.toLocaleString("en-US")}.00
                </span>
              </label>
              <label className="block">
                <span className="mb-2 block text-base font-black text-foreground">
                ...
                <div className="flex items-center gap-4 rounded-2xl border-[3px] border-border bg-tint-cream p-4 shadow-vibe-sm">
                  ...
                  <div className="flex-1">
                    <p className="text-base font-black text-foreground">
                      {bankDetails.method || "Bank account"}
                    </p>
                    <p className="text-sm font-bold text-foreground/70">
                      {bankDetails.bank_name || "No bank"} •{" "}
                      {bankDetails.account_number || "No account"}
                    </p>
                  </div>
                  <Link
                    to="/settings"
          ...
          <button
            ...
            className="mt-10 inline-flex w-full items-center justify-center gap-2 rounded-full border-[3px] border-border bg-primary py-4 text-lg font-black text-white shadow-vibe hover:-translate-y-1 hover:shadow-vibe-hover disabled:opacity-70 transition-all"
          >
        ...
        <aside className="rounded-[2.5rem] border-[4px] border-border bg-tint-lilac p-6 sm:p-8 shadow-vibe flex flex-col justify-center text-center">
          ...
          <p className="mt-6 inline-flex items-center justify-center rounded-full border-[3px] border-border bg-white px-4 py-2 text-sm font-black shadow-vibe-sm mx-auto">
              ...
              {history.length > 0 ? (
                history.map((h: Transaction) => (
                  <tr key={h.id} className="transition-colors hover:bg-muted/50">
                    ...
                    <td className="py-4">
                      <span
                        className={`rounded-full border-[3px] border-border px-3 py-1 text-xs font-black shadow-vibe-sm ${h.status === "paid" ? "bg-tint-mint" : "bg-tint-peach"}`}
                      >
                        {h.status}
                      </span>
                    </td>
