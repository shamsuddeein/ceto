...
export const Route = createFileRoute("/withdrawal-history")({
    ...
    <DashboardLayout title="Withdrawal History">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between rounded-[2.5rem] border-[4px] border-border bg-white p-8 shadow-vibe mb-10">
        <div>
          ...
          <p className="font-display text-4xl font-black text-foreground mt-1">₦{total.toLocaleString('en-US')}</p>
        </div>
        <Link to="/withdrawals" className="inline-flex items-center gap-2 rounded-full border-[3px] border-border bg-primary px-6 py-4 text-base font-black text-white shadow-vibe transition-transform hover:-translate-y-1 hover:shadow-vibe-hover">+ New withdrawal</Link>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-base">
            <thead className="text-left text-sm font-black uppercase tracking-wider text-foreground/60 border-b-[3px] border-border">
              <tr>
                <th className="px-5 py-4">ID</th><th className="py-4">Date</th><th className="py-4">Method</th><th className="py-4">Amount</th><th className="py-4">Status</th><th className="py-4">Receipt</th>
              </tr>
            </thead>
            <tbody className="divide-y-[3px] divide-border font-medium">
              {HISTORY.map((h) => (
                <tr key={h.id} className="transition-colors hover:bg-muted/50">
                  ...
                  <td className="font-semibold text-foreground">₦{h.amount.toLocaleString('en-US')}</td>
                  <td><span className="rounded-full border-[3px] border-border bg-tint-mint px-3 py-1 text-xs font-black shadow-vibe-sm text-foreground">{h.status}</span></td>
                  <td><button className="inline-flex items-center gap-2 rounded-xl border-[3px] border-border bg-tint-cream px-4 py-2 text-xs font-black text-foreground shadow-vibe-sm transition-transform hover:-translate-y-1 hover:bg-tint-peach"><Download className="h-4 w-4 stroke-[3px]" /> PDF</button></td>
                </tr>
              ))}
            </tbody>
          </table>
