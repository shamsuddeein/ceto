    ...
    <div className="min-h-screen bg-background">
      ...
      <main id="main-content" className="container-page flex min-h-[calc(100vh-4rem)] items-center justify-center py-10">
        <div className="w-full max-w-md rounded-2xl bg-card p-8 text-center shadow-[0_10px_40px_-10px_rgba(0,0,0,0.08)] sm:p-10">
          ...
          <h1 className="mt-4 font-display text-2xl font-bold text-primary sm:text-3xl">Verify your email</h1>
          <p className="mt-2 text-sm text-foreground/70">We sent a 6-digit code to your email. Enter it below to activate your account.</p>
          <form className="mt-6 space-y-5" onSubmit={verify} noValidate>
            <div className="flex justify-center gap-2 sm:gap-3">
              {code.map((d, i) => (
                <input
                  key={i} id={`otp-${i}`} value={d} onChange={(e) => setDigit(i, e.target.value)}
            ...
            <button type="submit" disabled={loading} className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-primary py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-70">
              {loading && <Loader2 className="h-4 w-4 animate-spin" />} {loading ? "Verifying..." : "Verify email"}
            </button>
          </form>
          <button onClick={resend} disabled={resending} className="mt-4 inline-flex items-center gap-1 text-sm text-foreground/70 hover:text-primary disabled:opacity-60">
            <RefreshCw className={`h-4 w-4 ${resending ? "animate-spin" : ""}`} /> Resend code
          </button>
          <p className="mt-6 text-xs text-foreground/60">Wrong email? <Link to="/signup" className="font-semibold text-primary hover:underline">Sign up again</Link></p>
        </div>
      </main>
    </div>
  );
