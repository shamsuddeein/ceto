...
import {
  Monitor, BookOpen, GraduationCap, Ticket, Star, Package,
  Share2, LayoutTemplate, MailCheck, Globe, ShieldCheck,
  BarChart3, Zap, ArrowRight, Check,
} from "lucide-react";
...
export const Route = createFileRoute("/features")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Everything you need to create, sell, and grow your digital products business. Explore all Cetoh features." },
    ],
  }),
      ...
      <main>
        ...
        <section className="relative overflow-hidden bg-background py-20 text-center md:py-32 border-b-[4px] border-border">
          ...
          <div className="container-page relative z-10">
            <p className="text-base font-black uppercase tracking-[0.25em] text-primary">FEATURES</p>
            <h1 className="mx-auto mt-6 max-w-5xl font-display text-5xl font-black text-foreground md:text-7xl leading-tight">
              Everything you need to <span className="text-primary relative inline-block">succeed<div className="absolute -bottom-2 left-0 right-0 h-3 rounded-full bg-tint-rose border-[3px] border-border shadow-vibe-sm -z-10 -rotate-2"></div></span> online
            </h1>
            <p className="mx-auto mt-8 max-w-2xl text-lg font-bold text-foreground/80 md:text-xl">
              Cetoh gives you a complete toolkit - from your first product upload to scaling your creator business in Nigeria.
            </p>
        ...
        <section className="bg-background py-20 md:py-28">
          <div className="container-page">
            <div className="mx-auto max-w-3xl text-center">
              <h2 className="font-display text-4xl font-black text-foreground md:text-6xl">All the features, zero compromises</h2>
            ...
            <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {categories.map((cat) => (
                <div key={cat.title} className={`rounded-[2.5rem] border-[4px] border-border bg-white p-8 shadow-vibe transition-transform hover:-translate-y-2`}>
                  <div className={`grid h-16 w-16 place-items-center rounded-[1rem] border-[3px] border-border ${cat.tint} shadow-vibe-sm`}>
                  ...
                  <ul className="mt-6 space-y-3">
                    {cat.items.map((item) => (
                      <li key={item} className="flex items-start gap-3 text-base font-bold text-foreground/90">
        ...
        <section className="bg-background py-20 border-t-[4px] border-border md:py-28">
          <div className="container-page">
            <h2 className="text-center font-display text-4xl font-black text-foreground md:text-6xl">Features built to make you money</h2>
            <div className="mt-16 grid gap-8 md:grid-cols-3">
              {highlights.map((h) => (
                <div key={h.title} className={`rounded-[2.5rem] border-[4px] border-border ${h.tint} p-8 shadow-vibe transition-transform hover:-translate-y-2`}>
                  ...
                  <p className="mt-4 text-[16px] font-bold leading-relaxed text-foreground/90">{h.body}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="bg-background py-16 md:py-24 border-y-[4px] border-border">
          <div className="container-page">
            <div className="mx-auto max-w-3xl text-center">
              <h2 className="font-display text-4xl font-black text-foreground md:text-5xl">Sell anything, to anyone</h2>
              <p className="mt-4 text-lg font-bold text-foreground/80">Six categories of products. One platform.</p>
            </div>
        ...
        <section className="bg-background py-20 text-center text-foreground overflow-hidden relative">
          ...
          <div className="container-page relative z-10">
            <h2 className="font-display text-4xl font-black md:text-6xl">Start using all these features today</h2>
