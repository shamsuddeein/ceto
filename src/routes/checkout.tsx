        ...
        <section className="container-page py-10">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_400px]">
            <form onSubmit={pay} className="space-y-6">
              ...
              <div className="rounded-2xl border border-border bg-card p-6">
                <h2 className="flex items-center gap-2 font-display text-lg font-semibold text-primary">
                  <ShieldCheck className="h-5 w-5" /> Secure Payment
                </h2>
              ...
              <button
                type="submit"
                disabled={loading}
                className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-primary py-4 text-base font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-70"
              >
                {loading ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" /> Connecting to Paystack...
                  </>
                ) : (
                  <>
                    <Lock className="h-4 w-4" /> Pay ₦{total.toLocaleString("en-US")} via Paystack
                  </>
                )}
              </button>
            ...
            <aside className="space-y-4">
              <div className="rounded-2xl border border-border bg-card p-6">
                ...
                <ul className="mt-4 space-y-3">
                  <li className="flex items-center gap-3">
                    <div
                      className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-md bg-muted overflow-hidden`}
                    >
                      {product.cover_image ? (
                        <img
                ...
                <div className="mt-5 space-y-2 border-t border-border pt-4 text-sm">
                  ...
                  <div className="flex justify-between pt-2 font-display text-base font-bold text-primary">
                    <span>Total</span>
                    <span>
                      {product.currency || "₦"}
                      {total.toLocaleString("en-US", { minimumFractionDigits: 2 })}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-foreground/70">
                        If you're not satisfied, we'll refund your purchase no questions asked.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <Link
                to="/marketplace"
                className="block text-center text-sm text-foreground/60 hover:text-primary"
              >
                ← Continue shopping
              </Link>
            </aside>
