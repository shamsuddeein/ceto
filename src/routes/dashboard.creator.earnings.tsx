    ...
    <DashboardLayout title="Earnings">
      <div className="grid gap-6 sm:grid-cols-3">
        <Stat
          ...
          value={`₦${Number(wallet?.available_balance || 0).toLocaleString("en-US")}`}
          tone="primary"
        />
        <Stat
          icon={TrendingUp}
          ...
          <table className="min-w-full text-base">
            <thead className="text-left text-sm font-black uppercase tracking-wider text-foreground/60 border-b-[3px] border-border">
              <tr>
                ...
                <th className="py-4">Gross</th>
                <th className="py-4">Fee</th>
                <th className="py-4">Net</th>
              </tr>
            </thead>
...
function Stat({
  ...
  return (
    <div
      className={`rounded-[2rem] border-[4px] border-border p-6 shadow-vibe transition-transform hover:-translate-y-1 ${isPrimary ? "bg-tint-mint" : "bg-white"}`}
    >
      <div
        className={`inline-flex rounded-xl border-[3px] border-border p-2 shadow-vibe-sm ${isPrimary ? "bg-white" : "bg-tint-lilac"}`}
      >
