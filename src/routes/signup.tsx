...
function SignupPage() {
  ...
  const [role, setRole] = useState<"" | "customer" | "creator">("");
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!role) return toast.error("Please select whether you are a Customer or Creator.");
        ...
        <section className="px-4 py-8 sm:px-6 md:px-12 md:py-10 lg:py-16">
          <div className="mx-auto w-full max-w-md">
            ...
            <form className="mt-6 space-y-4 sm:mt-8" onSubmit={handleSubmit} noValidate>
              {/* Account type */}
              <div>
                <label htmlFor="signup-role" className="block text-sm font-bold text-foreground">
                  I am a…
                </label>
                <div className="relative mt-2">
                  <select
                    id="signup-role"
                    value={role}
                    onChange={(e) => setRole(e.target.value as "" | "customer" | "creator")}
                    disabled={loading}
                    className={`w-full appearance-none rounded-2xl border-[3px] border-border bg-background px-4 py-4 pr-10 font-bold text-foreground outline-none shadow-vibe-sm transition-all focus:translate-x-[2px] focus:translate-y-[2px] focus:shadow-none disabled:opacity-60 ${
                ...
                {role === "customer" && (
                  <p className="mt-2 text-xs font-bold text-foreground/60">
                    Browse thousands of digital products from top Nigerian creators.
                  </p>
                )}
              </div>
        ...
        <section className="relative hidden overflow-hidden bg-primary-soft/40 px-4 py-10 sm:px-6 md:px-10 lg:block lg:py-16">
          ...
          <div className="relative mx-auto max-w-xl text-center">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm text-foreground/80">
              Sell digital products and services effortlessly and enjoy 0% commission for your first
              3 months.
            </p>
          </div>
