  ...
  head: () => ({
    meta: [
      { title: "Marketplace | Discover digital products | Cetoh" },
      {
        name: "description",
        content:
          "Browse thousands of eBooks, courses, templates and services from Nigerian creators.",
      },
    ],
  }),
          ...
          <div className="container-page relative z-10 py-16 md:py-24">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {categories.slice(0, 6).map((c) => (
                <span
                  key={c.slug}
                  className="rounded-full border-[3px] border-border bg-white px-5 py-2 text-sm font-black text-foreground shadow-vibe-sm transition-transform hover:-translate-y-1 hover:bg-tint-cream cursor-pointer"
                >
                  {c.name}
                </span>
              ))}
